...
class TabPaletteModal extends Modal {
	constructor(app, plugin) {
		...
		this.tabs = [];
		this.bookmarks = [];
		this.allItems = []; // List merging tabs and bookmarks.
	}
	...
	renderTabs(container) {
		...
		this.tabs.forEach((tab, index) => {
			...
			if (this.plugin.settings.showPath) {
				...
				pathParts.pop(); // Excludes the last element (the file name).
	...
	closeSelectedTab() {
		const selectedItem = this.allItems[this.selectedIndex];
		if (!selectedItem || selectedItem.type !== 'tab') return; // Only tabs can be closed.
	...
	pinSelectedTab() {
		const selectedItem = this.allItems[this.selectedIndex];
		if (!selectedItem || selectedItem.type !== 'tab') return; // Only tabs can be pinned.
