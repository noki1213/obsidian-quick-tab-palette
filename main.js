...
const DEFAULT_SETTINGS = {
	...
	deduplicateTabs: true, // Prevent duplicate tabs: switch to the existing tab if the same file is already open.
...
class TabPaletteModal extends Modal {
	constructor(app, plugin) {
		...
		this.searchQuery = '';
		this.vaultFiles = []; // Cache of all files.
		this.fileContentCache = new Map(); // File contents cache (path → lowercased contents).
		this.fileContentCacheOriginal = new Map(); // Original contents (case preserved).
		this.searchDebounceTimer = null; // Timer used for debouncing.
	...
	async onOpen() {
		...
		this.searchResults = []; // Whether to start empty or show all items. Here it starts empty.
	...
	switchSection(direction) {
		...
		if (this.activeSection !== nextSection) {
			...
			let targetColumnIndex = -1;
			if (nextSection === 'search') targetColumnIndex = 0; // Always left.
	...
	getTabs() {
		...
		recentlyClosed.forEach(closedTab => {
			...
			if (!isExcluded) {
				tabs.push({
					leaf: null, // No leaf, since this is a closed tab.
					file: file,
					name: file.basename,
					path: file.path,
					isPinned: false,
	...
	pinSelectedTab() {
		...
		tab.isPinned = !tab.isPinned; // Local update.
	...
	getDailyNotes() {
		...
		dates.forEach(({ label, date }) => {
			...
			dailyNotes.push({
				...
				exists: !!file, // Flag for whether the file exists.
				momentDate: date // Used at creation time.
			});
		});
		...
		new Setting(containerEl)
			...
			.addToggle(toggle => toggle
				...
				.onChange(async (value) => {
					...
					await this.plugin.saveSettings();
					this.display(); // Re-render to update the advanced-settings display state.
				}));
		...
		new Setting(containerEl)
			...
			.addToggle(toggle => toggle
				...
				.onChange(async (value) => {
					...
					await this.plugin.saveSettings();
					this.display(); // Update the display of related settings.
				}));
